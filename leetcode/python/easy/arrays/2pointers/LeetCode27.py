...
class LeetCode27(object):
    def removeElement(self, nums, val):
        ...
        for i in range(len(nums)):
            # If current number is not we want to remove, then we can
			# consider it in the final array and add it to the current
			# position ptr and increment ptr to fill the next position
            if nums[i] != val:
                nums[ptr] = nums[i]
                ptr+=1
        # Once all the numbers have been iterated through, ptr will
		# be at 1 position forward to last filled position and
		# hence ptr will actually signify the length of the good
		# array instead of index
        return ptr
